import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Hello world!
 *
 */
public class App {
	private final static Logger logger = LoggerFactory.getLogger(App.class);

	public static void main(String[] args) {
		logger.info("Launching QuPath with args: {}", String.join(", ", args));
		if (args != null && args.length > 0 && "tma".equals(args[0].toLowerCase())) {

		}
		// 시작 프로젝트
		else {
			// // This was an attempt to register a file listener at an early stage, so as
			// to be able to launch QuPath
			// by double-clicking on an associated file.
			// Kind of worked, but somehow ended up thwarting attempts to set the system
			// menubar.
			// if (GeneralTools.isMac()) {
			// try {
			// Class<?> osx = Class.forName("qupath.extensions.osx.OSXExtensions", true,
			// QuPathGUI.getClassLoader());
			// if (osx != null) {
			// Method method = osx.getMethod("setOSXFileHandler");
			// method.invoke(null);
			// }
			// } catch (Exception e) {
			// logger.debug("Error setting OSX extensions!", e);
			// }
			// }
		}
	}
}
